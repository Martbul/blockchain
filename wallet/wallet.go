...
func NewWallet() *Wallet {
	...
	if err != nil {
		fmt.Errorf("Some error generating keys %s", err)
	}
	...
	h3 := ripemd160.New() //ripemd will result in shorter hash then sha-256
	h3.Write(digest2)
	digest3 := h3.Sum(nil)
	...
	// 8. Addd the 4 chacksum bytes from 7 at the end of extended RIPEMD-160 hash from 4 (25 bytes)
	dc8 := make([]byte, 25)
	copy(dc8[:21], vd4[:])
	copy(dc8[21:], chsum[:])
